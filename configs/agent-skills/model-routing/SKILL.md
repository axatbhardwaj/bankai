...
- For authored-PR babysitting to merge-ready, use [paseo-pr-babysit](../paseo-pr-babysit/SKILL.md): Opus monitors, Sol repairs, and Opus reviews before the driver publishes. Its monitor heartbeat is owned by the Opus session.
...
The legacy profile IDs `docs-glm`, `pr-correctness-grok`, `pr-requirements-glm`, and `pr-monitor-glm` are stable workflow keys; their configured runtime is Claude Opus 5 at medium effort.
