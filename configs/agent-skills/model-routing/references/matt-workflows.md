...
3. The driver dispatches `review-opus` against the candidate in a separate pinned checkout. Opus executes the `implement` skill's `code-review` step once, including its native Standards and Spec subagents. Astra and Sol do not run a second full review or replace it with the five-angle peer-PR workflow.
